...
import { getHomeFeed, notifications, type Category, type HomeFeed, type Product } from "@/shared";

/**
 * الشاشة الرئيسية — مطابقة لرئيسية الويب (customer.index.tsx):
 * ترويسة بحث، بنر ترحيبي، الفئات، ثم أقسام المنتجات الأربعة.
 */
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    getHomeFeed().then((res) => {
      if (alive && res.ok) setFeed(res.data);
    });
    return () => {
      alive = false;
    };
    ...
    <SafeAreaView edges={["top"]} style={styles.screen}>
      ...
      {!feed ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          <HeroBanner />
          <CategoriesRow categories={feed.categories} />
          <ProductSection
            title="منتجات مميّزة"
...
function HomeHeader() {
  ...
  return (
    <View style={styles.header}>
      <BrandMark size={34} />
      {/* البحث الكامل ضمن مهام اليوم 8 — حاليًا يفتح تصفّح الفئات */}
      <Pressable
